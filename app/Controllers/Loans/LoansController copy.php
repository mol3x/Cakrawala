    ...
    public function edit($uid = null)
{
    ...
    if ($this->request->getMethod() === 'post') {
        $newDuration = $this->request->getVar('duration');
        if (!is_numeric($newDuration) || $newDuration <= 0 || $newDuration > 30) {
            return redirect()->back()->with('error', 'Invalid duration. It must be a number between 1 and 30 days.');
        }

        $newDueDate = Time::createFromFormat('Y-m-d H:i:s', $loan['loan_date'])
            ->addDays(intval($newDuration))
            ->toDateTimeString();

        $this->loanModel->update($loan['id'], ['due_date' => $newDueDate]);

        return redirect()->to("admin/loans/{$loan['uid']}")->with('success', 'Duration updated successfully.');
    }
...
public function update($uid = null)
{
    if ($this->request->getMethod() !== Method::PUT && $this->request->getMethod() !== Method::POST) {
        throw new PageNotFoundException('Invalid request method');
    }

    // Temukan data peminjaman berdasarkan UID
    $loan = $this->loanModel->where('uid', $uid)->first();
    ...
    // Validasi input durasi baru
    $validation = $this->validate([
        'duration' => 'required|numeric|integer|greater_than[0]|less_than_equal_to[30]', // Maksimum 30 hari
    ]);
    ...
    // Update due_date berdasarkan durasi baru
    $updatedDueDate = Time::createFromFormat('Y-m-d H:i:s', $loan['loan_date'])->addDays(intval($newDuration))->toDateTimeString();

    $this->loanModel->update($loan['id'], ['due_date' => $updatedDueDate]);

    // Set flashdata untuk pesan sukses
    session()->setFlashdata('success', 'Loan duration updated successfully.');

    // Redirect ke halaman hasil, misalnya halaman detail peminjaman
    return redirect()->to(base_url('admin/loans/' . $uid));
}
